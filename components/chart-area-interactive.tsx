...
export function ChartAreaInteractive() {
  ...
  // Procesar facturas para la gráfica
  const chartData = React.useMemo(() => {
    ...
    facturas.forEach((f) => {
      if (!f.fecha) return
      const date = f.fecha
      const current = dataMap.get(date) || { desktop: 0, mobile: 0 }
      
      dataMap.set(date, {
        desktop: current.desktop + f.totalRef, // Monto en $
        mobile: current.mobile + f.cantidad    // Cantidad de facturas/items
      })
    })
  ...
  const filteredData = chartData.filter((item) => {
    ...
    const referenceDate = new Date() // Usar fecha actual
